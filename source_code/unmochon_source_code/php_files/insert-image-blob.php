...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($_FILES["photo"]) && $_FILES["photo"]["error"] == 0){
        ...
        if(in_array($filetype, $allowed)){
            ...
            $query = "INSERT INTO image_list(image_name,image_blob) VALUES('".$filename."','".addslashes(file_get_contents($_FILES["photo"]["tmp_name"]))."')";
            if(mysqli_query($connect, $query))
            {
                echo "inserted screenshot.".$filename."\n";
            }
            else echo "could not insert into screenshot".$filename."\n";
            mysqli_close($connect);

//            echo "Your file was uploaded successfully." .$filename."\n"; // "." is the separator used to determine filename in java
        } else{
